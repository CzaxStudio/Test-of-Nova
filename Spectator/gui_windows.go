...
func buildPage() string {
	...
	sbCSS := ""
	if !gui.scrollbar {
		sbCSS = "::-webkit-scrollbar{display:none}"
	}
...
function _sSbNav(id){
  document.querySelectorAll('.g-sb-item').forEach(el=>el.classList.remove('active'));
  const el=document.getElementById('sb_'+id);if(el)el.classList.add('active');
  try{window._goTriggerEvent('sidebar:'+id);}catch(e){}
}
function _sSlider(id,val,show){
  try{window._goUpdateValue(id,val);}catch(e){}
  if(show){const e=document.getElementById('slv_'+id);if(e)e.textContent=val;}
}
function _trig(id){
  try{window._goTriggerEvent(id);}catch(e){}
}
...
func renderWidget(w GUIWidget) string {
	...
	case "input":
		...
		val := gS(o, "value", "")
		dis := ""
		if gB(o, "disabled", false) {
			dis = " disabled"
		}
		if gB(o, "multiline", false) {
			rows := int(gF(o, "rows", 4))
			return fmt.Sprintf(`<textarea class="g-input" id="inp_%s" placeholder="%s" rows="%d" style="width:%s"%s oninput="_upd('%s',this.value)">%s</textarea>`, w.ID, he(ph), rows, wide, dis, w.ID, he(val))
		}
	...
	case "password":
		ph := gS(o, "placeholder", "")
		return fmt.Sprintf(`<input class="g-input" type="password" id="inp_%s" placeholder="%s" oninput="_upd('%s',this.value)">`, w.ID, he(ph), w.ID)
	case "number":
		ph := gS(o, "placeholder", "0")
		val := gS(o, "value", "")
		step := gS(o, "step", "1")
		mn := ""
		if v := gS(o, "min", ""); v != "" {
	...
	case "button":
		clr := gS(o, "color", "var(--ac)")
		tc := gS(o, "textColor", "#fff")
		wide := gS(o, "width", "auto")
		sz := gS(o, "size", "")
		icon := gS(o, "icon", "")
		dis := ""
		if gB(o, "disabled", false) {
		...
		outC := ""
		bgS := fmt.Sprintf("background:%s;color:%s;", clr, tc)
		if gB(o, "outline", false) {
			outC = " g-btn-outline"
			bgS = fmt.Sprintf("--bc:%s;", clr)
		}
		iconH := ""
		if icon != "" {
			iconH = fmt.Sprintf(`<span>%s</span>`, icon)
		}
	...
	case "link":
		clr := gS(o, "color", "var(--ac)")
		return fmt.Sprintf(`<button class="g-link" style="color:%s" onclick="_trig('%s')">%s</button>`, clr, w.EventID, he(w.Text))
	case "output":
		h := gF(o, "height", 300)
		bg := gS(o, "bg", "")
		clr := gS(o, "color", "")
		sty := fmt.Sprintf("height:%.0fpx;", h)
		if bg != "" {
			sty += "background:" + bg + ";"
		}
		if clr != "" {
	...
	case "progress":
		...
		lblH := ""
		if lbl != "" {
			lblH = fmt.Sprintf(`<div class="g-prog-label">%s</div>`, he(lbl))
		}
	...
	case "checkbox":
		chkd := ""
		if gB(o, "checked", false) {
			chkd = " checked"
		}
		clr := gS(o, "color", "var(--tx)")
		return fmt.Sprintf(`<label class="g-check-wrap" style="color:%s"><input type="checkbox" id="inp_%s"%s onchange="_upd('%s',this.checked?'true':'false')"> %s</label>`, clr, w.ID, chkd, w.ID, he(w.Text))
	case "toggle":
		chkd := ""
		if gB(o, "checked", false) {
			chkd = " checked"
		...
		clr := gS(o, "color", "var(--tx)")
		return fmt.Sprintf(`<div class="g-toggle-wrap"><div class="g-toggle-track"><input type="checkbox" class="g-toggle-input" id="inp_%s"%s onchange="_upd('%s',this.checked?'true':'false')"><div class="g-toggle-thumb"></div></div><span style="color:%s">%s</span></div>`, w.ID, chkd, w.ID, clr, he(w.Text))
	case "radio":
		nm := gS(o, "name", "radio")
		val := gS(o, "value", w.Text)
		chkd := ""
		if gB(o, "checked", false) {
			chkd = " checked"
		}
	...
	case "dropdown":
		...
		if ph != "" {
			sb.WriteString(fmt.Sprintf(`<option value="" disabled>%s</option>`, he(ph)))
		}
		if list, ok := o["options"].([]interface{}); ok {
			for i, item := range list {
				sel := ""
				if i == 0 && ph == "" {
					sel = " selected"
				}
				sb.WriteString(fmt.Sprintf(`<option value="%s"%s>%s</option>`, he(toStr(item)), sel, he(toStr(item))))
			}
			if len(list) > 0 && ph == "" {
				gui.mu.Lock()
				if gui.values[w.ID] == "" {
					gui.values[w.ID] = toStr(list[0])
				}
				gui.mu.Unlock()
			}
		}
	...
	case "slider":
		...
		wide := gS(o, "width", "100%%")
		clr := gS(o, "color", "")
		show := gB(o, "showValue", false)
		sty := fmt.Sprintf("width:%s;", wide)
		if clr != "" {
			sty += "accent-color:" + clr + ";"
		}
		sv := ""
		if show {
			sv = fmt.Sprintf(`<div class="g-slider-val" id="slv_%s">%.0f</div>`, w.ID, mn)
		}
		bv := "false"
		if show {
			bv = "true"
	...
	case "table":
		h := gF(o, "height", 0)
		striped := gB(o, "striped", true)
		hSty := ""
		if h > 0 {
			hSty = fmt.Sprintf("max-height:%.0fpx;", h)
		}
		stpC := ""
		if striped {
			stpC = " g-table-striped"
		}
		var sb strings.Builder
		sb.WriteString(fmt.Sprintf(`<div class="g-table-wrap" style="%s"><table class="g-table%s"><thead><tr>`, hSty, stpC))
		if hdrs, ok := o["headers"].([]interface{}); ok {
			for _, h := range hdrs {
				sb.WriteString(fmt.Sprintf(`<th>%s</th>`, he(toStr(h))))
			}
		}
		sb.WriteString(fmt.Sprintf(`</tr></thead><tbody id="tb_%s">`, w.ID))
		if rows, ok := o["rows"].([]interface{}); ok {
			for _, row := range rows {
				sb.WriteString(`<tr>`)
				if cells, ok2 := row.([]interface{}); ok2 {
					for _, c := range cells {
						sb.WriteString(fmt.Sprintf(`<td>%s</td>`, he(toStr(c))))
					}
				}
				sb.WriteString(`</tr>`)
			}
	...
	case "divider":
		clr := gS(o, "color", "var(--bd)")
		mg := gS(o, "margin", "10px 0")
		txt := gS(o, "text", "")
		if txt != "" {
			return fmt.Sprintf(`<div class="g-div-text" style="margin:%s;color:%s">%s</div>`, mg, clr, he(txt))
		}
	...
	case "card":
		...
		if bd := gS(o, "border", ""); bd != "" {
			sty += "border-color:" + bd + ";"
		}
		if r := gS(o, "radius", ""); r != "" {
			sty += "border-radius:" + r + "px;"
		}
		if p := gS(o, "padding", ""); p != "" {
			sty += "padding:" + p + "px;"
		}
		acC := ""
		if gB(o, "accent", false) {
			acC = " g-card-accent"
		}
		id := gS(o, "id", fmt.Sprintf("card%d", len(gui.widgets)))
		title := gS(o, "title", "")
		titleH := ""
		if title != "" {
			titleH = fmt.Sprintf(`<div class="g-card-title">%s</div>`, he(title))
		}
	...
	case "badge":
		clr := gS(o, "color", "#fff")
		bg := gS(o, "bg", "var(--ac)")
		rad := gS(o, "radius", "999px")
		return fmt.Sprintf(`<span class="g-badge" style="color:%s;background:%s;border-radius:%s">%s</span>`, clr, bg, rad, he(w.Text))
	case "alert_box":
		kind := gS(o, "type", "info")
		id := gS(o, "id", "")
		icons := map[string]string{"info": "ℹ", "success": "✓", "warning": "⚠", "error": "✕"}
		icon := icons[kind]
		if icon == "" {
			icon = "ℹ"
		}
	...
	case "code":
		...
		sty := ""
		if h > 0 {
			sty = fmt.Sprintf("max-height:%.0fpx;overflow-y:auto;", h)
		}
		if bg := gS(o, "bg", ""); bg != "" {
			sty += "background:" + bg + ";"
		}
	...
	case "tabs":
		grp := gS(o, "group", "default")
		var sb strings.Builder
		sb.WriteString(`<div class="g-tabs">`)
		if list, ok := o["tabs"].([]interface{}); ok {
			for i, t := range list {
				act := ""
				if i == 0 {
					act = " active"
				...
				tid := toStr(t)
				sb.WriteString(fmt.Sprintf(`<button class="g-tab-btn%s" data-tab="%s" data-group="%s" onclick="_sOpenTab('%s','%s')">%s</button>`, act, tid, grp, tid, grp, he(tid)))
			}
		}
		sb.WriteString(`</div>`)
		return sb.String()
	...
	case "rowStart":
		gap := gS(o, "gap", "8px")
		align := gS(o, "align", "center")
		just := gS(o, "justify", "flex-start")
	...
	case "sidebar":
		var sb strings.Builder
		sb.WriteString(`<div class="g-layout"><div class="g-sidebar">`)
		if items, ok := o["items"].([]interface{}); ok {
			for i, item := range items {
				if m, ok2 := item.(map[string]interface{}); ok2 {
					...
					iconH := ""
					if icon != "" {
						iconH = fmt.Sprintf(`<span class="g-sb-icon">%s</span>`, icon)
					}
					sb.WriteString(fmt.Sprintf(`<button class="g-sb-item%s" id="sb_%s" onclick="_sSbNav('%s')">%s<span>%s</span></button>`, act, id, id, iconH, he(lbl)))
				}
			}
		}
		sb.WriteString(`</div><div class="g-main">`)
		return sb.String()
	case "sidebarEnd":
		return `</div></div>`
	case "header":
		title := gS(o, "title", w.Text)
		sub := gS(o, "subtitle", "")
		logo := gS(o, "logo", "")
		clr := gS(o, "color", "var(--tx)")
		bg := gS(o, "bg", "")
		sty := fmt.Sprintf("color:%s;", clr)
		if bg != "" {
			sty += "background:" + bg + ";"
		}
		logoH := ""
		if logo != "" {
			logoH = fmt.Sprintf(`<img class="g-header-logo" src="%s">`, embedImg(logo))
		}
		subH := ""
		if sub != "" {
			subH = fmt.Sprintf(`<div class="g-header-sub">%s</div>`, he(sub))
		}
...
func (interp *Interpreter) guiBuiltin(name string, args []interface{}, argExprs []Expr, env *Env) (interface{}, error) {
	aa := func(idx int, def string) string {
		if idx < len(args) {
			return toStr(args[idx])
		}
		return def
	}
	om := func(idx int) map[string]interface{} {
		if idx < len(args) {
	...
	merge := func(base map[string]interface{}, idx int) map[string]interface{} {
		if idx < len(args) {
			if m, ok := args[idx].(map[string]interface{}); ok {
				for k, v := range m {
					base[k] = v
	...
	case "open.window":
		...
		if v, ok := o["width"]; ok {
			gui.width = int(toFloat(v))
		}
		if v, ok := o["height"]; ok {
			gui.height = int(toFloat(v))
		}
		if v, ok := o["bg"]; ok {
			gui.bg = toStr(v)
		}
		...
		if v, ok := o["padding"]; ok {
			gui.padding = toStr(v)
		}
		if v, ok := o["resizable"]; ok {
			gui.resizable = isTruthy(v)
		}
		if v, ok := o["scrollbar"]; ok {
			gui.scrollbar = isTruthy(v)
		}
		if v, ok := o["css"]; ok {
			gui.customCSS = toStr(v)
		}
		gui.ready = true
		return nil, nil
	...
	case "end":
		if !gui.ready {
			return nil, fmt.Errorf("call open.window() before end()")
		}
		wv := webview.New(true)
		defer wv.Destroy()
		gui.wv = wv
		...
		wv.SetSize(gui.width, gui.height, hint)
		// Bind Go functions BEFORE loading HTML — this is critical.
		// WebView2 Bind() injects the function into every page that loads after this call.
		wv.Bind("_goUpdateValue", func(id, val string) {
			gui.mu.Lock()
			gui.values[id] = val
			gui.mu.Unlock()
		})
		wv.Bind("_goTriggerEvent", func(id string) {
			gui.mu.Lock()
			fn, ok := gui.events[id]
			gui.mu.Unlock()
			if ok {
		...
		wv.Bind("_goConfirmResult", func(val string) {
			select {
			case gui.confirm <- (val == "true"):
			default:
			}
		})
		gui.safeEval = func(js string) { wv.Dispatch(func() { wv.Eval(js) }) }
		// SetHtml after Bind so all functions are available when page JS runs
		wv.SetHtml(buildPage())
		wv.Run()
		return nil, nil
	...
	case "GUI.label":
		...
		add(GUIWidget{Kind: "label", Text: aa(0, ""), Opts: om(1)})
		return nil, nil
	case "GUI.input":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.input requires id")
		}
		id := aa(0, "")
		o := map[string]interface{}{}
		if len(args) >= 2 {
		...
		o = merge(o, 2)
		gui.values[id] = gS(o, "value", "")
		add(GUIWidget{ID: id, Kind: "input", Opts: o})
		return nil, nil
	case "GUI.password":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.password requires id")
		}
		id := aa(0, "")
		o := map[string]interface{}{}
		if len(args) >= 2 {
		...
		gui.values[id] = ""
		add(GUIWidget{ID: id, Kind: "password", Opts: o})
		return nil, nil
	case "GUI.number":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.number requires id")
		}
		id := aa(0, "")
		o := map[string]interface{}{}
		if len(args) >= 2 {
		...
		o = merge(o, 2)
		gui.values[id] = gS(o, "value", "0")
		add(GUIWidget{ID: id, Kind: "number", Opts: o})
		return nil, nil
	case "GUI.button":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.button requires text, event")
		}
		add(GUIWidget{ID: aa(1, ""), Kind: "button", Text: aa(0, ""), EventID: aa(1, ""), Opts: om(2)})
		return nil, nil
	case "GUI.iconButton":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.iconButton requires icon, event")
		}
		add(GUIWidget{ID: aa(1, ""), Kind: "iconButton", Text: aa(0, ""), EventID: aa(1, ""), Opts: om(2)})
		return nil, nil
	case "GUI.link":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.link requires text, event")
		}
		add(GUIWidget{ID: aa(1, ""), Kind: "link", Text: aa(0, ""), EventID: aa(1, ""), Opts: om(2)})
		return nil, nil
	case "GUI.output":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.output requires id")
		}
		id := aa(0, "")
		gui.outputs[id] = []string{}
		add(GUIWidget{ID: id, Kind: "output", Opts: om(1)})
		return nil, nil
	case "GUI.progress":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.progress requires id")
		}
		add(GUIWidget{ID: aa(0, ""), Kind: "progress", Opts: om(1)})
		return nil, nil
	case "GUI.spinner":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.spinner requires id")
		}
		add(GUIWidget{ID: aa(0, ""), Kind: "spinner"})
		return nil, nil
	case "GUI.checkbox":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.checkbox requires id, label")
		}
		id := aa(0, "")
		o := om(2)
		gui.values[id] = "false"
		...
		add(GUIWidget{ID: id, Kind: "checkbox", Text: aa(1, ""), Opts: o})
		return nil, nil
	case "GUI.toggle":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.toggle requires id, label")
		}
		id := aa(0, "")
		o := om(2)
		gui.values[id] = "false"
		...
		add(GUIWidget{ID: id, Kind: "toggle", Text: aa(1, ""), Opts: o})
		return nil, nil
	case "GUI.radio":
		if len(args) < 3 {
			return nil, fmt.Errorf("GUI.radio requires id, name, label")
		}
		id := aa(0, "")
		o := map[string]interface{}{"name": aa(1, ""), "value": aa(3, aa(2, ""))}
		o = merge(o, 4)
		add(GUIWidget{ID: id, Kind: "radio", Text: aa(2, ""), Opts: o})
		return nil, nil
	case "GUI.dropdown":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.dropdown requires id, options")
		}
		id := aa(0, "")
		o := map[string]interface{}{"options": args[1]}
		o = merge(o, 2)
		add(GUIWidget{ID: id, Kind: "dropdown", Opts: o})
		return nil, nil
	case "GUI.slider":
		if len(args) < 3 {
			return nil, fmt.Errorf("GUI.slider requires id, min, max")
		}
		id := aa(0, "")
		o := map[string]interface{}{"min": args[1], "max": args[2]}
		o = merge(o, 3)
		gui.values[id] = toStr(args[1])
		add(GUIWidget{ID: id, Kind: "slider", Opts: o})
		return nil, nil
	case "GUI.table":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.table requires id")
		}
		id := aa(0, "")
		o := map[string]interface{}{}
		if len(args) >= 2 {
		...
		o = merge(o, 3)
		gui.tables[id] = [][]string{}
		add(GUIWidget{ID: id, Kind: "table", Opts: o})
		return nil, nil
	case "GUI.image":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.image requires path")
		}
		add(GUIWidget{Kind: "image", Text: aa(0, ""), Opts: om(1)})
		return nil, nil
	case "GUI.space":
		h := float64(16)
		if len(args) >= 1 {
		...
		add(GUIWidget{Kind: "space", Opts: map[string]interface{}{"height": h}})
		return nil, nil
	case "GUI.divider":
		add(GUIWidget{Kind: "divider", Opts: om(0)})
		return nil, nil
	case "GUI.card":
		title := ""
		if len(args) >= 1 {
		...
		o := map[string]interface{}{"title": title}
		o = merge(o, 1)
		id := gS(o, "id", fmt.Sprintf("card%d", len(gui.widgets)))
		o["id"] = id
		add(GUIWidget{ID: id, Kind: "card", Opts: o})
		return nil, nil
	case "GUI.cardEnd":
		add(GUIWidget{Kind: "cardEnd"})
		return nil, nil
	case "GUI.badge":
		if len(args) < 1 {
			return nil, nil
		}
		add(GUIWidget{Kind: "badge", Text: aa(0, ""), Opts: om(1)})
		return nil, nil
	case "GUI.alert_box":
		if len(args) < 1 {
			return nil, nil
		...
		add(GUIWidget{Kind: "alert_box", Text: aa(0, ""), Opts: o})
		return nil, nil
	case "GUI.code":
		if len(args) < 1 {
			return nil, nil
		}
		add(GUIWidget{Kind: "code", Text: aa(0, ""), Opts: om(1)})
		return nil, nil
	case "GUI.tabs":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.tabs requires tab list")
		}
		add(GUIWidget{Kind: "tabs", Opts: map[string]interface{}{"tabs": args[0]}})
		return nil, nil
	case "GUI.tabPanel":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.tabPanel requires id")
		}
		add(GUIWidget{ID: aa(0, ""), Kind: "tabPanel", Opts: om(1)})
		return nil, nil
	case "GUI.tabPanelEnd":
		add(GUIWidget{Kind: "tabPanelEnd"})
		return nil, nil
	case "GUI.rowStart":
		add(GUIWidget{Kind: "rowStart", Opts: om(0)})
		return nil, nil
	case "GUI.rowEnd":
		add(GUIWidget{Kind: "rowEnd"})
		return nil, nil
	case "GUI.colStart":
		add(GUIWidget{Kind: "colStart", Opts: om(0)})
		return nil, nil
	case "GUI.colEnd":
		add(GUIWidget{Kind: "colEnd"})
		return nil, nil
	case "GUI.sidebar":
		if len(args) < 1 {
			return nil, fmt.Errorf("GUI.sidebar requires items")
		}
		add(GUIWidget{Kind: "sidebar", Opts: map[string]interface{}{"items": args[0]}})
		return nil, nil
	case "GUI.sidebarEnd":
		add(GUIWidget{Kind: "sidebarEnd"})
		return nil, nil
	case "GUI.header":
		text := ""
		if len(args) >= 1 {
		...
		o := map[string]interface{}{"title": text}
		o = merge(o, 1)
		add(GUIWidget{Kind: "header", Text: text, Opts: o})
		return nil, nil
	case "GUI.footer":
		add(GUIWidget{Kind: "footer", Text: aa(0, "")})
		return nil, nil
	case "GUI.html":
		if len(args) >= 1 {
			add(GUIWidget{Kind: "html", Text: aa(0, "")})
		}
		return nil, nil
	case "GUI.on":
		if len(args) < 2 {
			return nil, fmt.Errorf("GUI.on requires event, func")
		}
		event := aa(0, "")
		fn, ok := args[1].(*UserFunc)
		if !ok {
			return nil, fmt.Errorf("GUI.on: second arg must be func")
		}
		child := NewEnv(fn.Env)
		gui.mu.Lock()
		gui.events[event] = func() { interp.execBlock(fn.Body, child) }
		gui.mu.Unlock()
		return nil, nil
	case "GUI.get":
		if len(args) < 1 {
	...
	case "GUI.set":
		...
		gui.mu.Unlock()
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sSetValue(%q,%q)`, id, val))
		}
		return nil, nil
	case "GUI.print", "GUI.println":
		...
		text := aa(1, "")
		gui.mu.Lock()
		gui.outputs[id] = append(gui.outputs[id], text)
		combined := strings.Join(gui.outputs[id], "\n")
		gui.mu.Unlock()
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sSetOutput(%q,%q)`, id, combined))
		}
		return nil, nil
	case "GUI.clear":
		if len(args) < 1 {
		...
		gui.mu.Unlock()
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sSetOutput(%q,"")`, id))
			gui.safeEval(fmt.Sprintf(`_sSetValue(%q,"")`, id))
		}
		return nil, nil
	case "GUI.setProgress":
		...
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sSetProgress(%q,%.4f)`, aa(0, ""), toFloat(args[1])))
		}
		return nil, nil
	case "GUI.appendRow":
		if len(args) < 2 {
		...
		id := aa(0, "")
		var cells []string
		if row, ok := args[1].([]interface{}); ok {
			for _, c := range row {
				cells = append(cells, toStr(c))
			}
		}
		gui.mu.Lock()
		gui.tables[id] = append(gui.tables[id], cells)
		gui.mu.Unlock()
		if gui.safeEval != nil {
			var jc []string
			for _, c := range cells {
				jc = append(jc, fmt.Sprintf("%q", c))
			}
			gui.safeEval(fmt.Sprintf(`_sAppendRow(%q,[%s])`, id, strings.Join(jc, ",")))
		}
		return nil, nil
	case "GUI.clearTable":
		if len(args) < 1 {
		...
		gui.mu.Unlock()
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sClearTable(%q)`, id))
		}
		return nil, nil
	case "GUI.show":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sShow(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.hide":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sHide(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.enable":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sEnable(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.disable":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sDisable(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.focus":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sFocus(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.showSpinner":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sShowSpin(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.hideSpinner":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sHideSpin(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.setTitle":
		if len(args) >= 1 && gui.wv != nil {
			gui.wv.Dispatch(func() { gui.wv.SetTitle(aa(0, "")) })
		}
		return nil, nil
	case "GUI.setAccent":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sAccent(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.setBg":
		if len(args) >= 1 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sBg(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.alert":
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sAlert(%q)`, aa(0, "")))
		}
		return nil, nil
	case "GUI.confirm":
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sConfirm(%q)`, aa(0, "")))
		}
		select {
		case result := <-gui.confirm:
			return result, nil
		}
	case "GUI.notify":
		if gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`if(Notification.permission==="granted"){new Notification(%q,{body:%q})}`, aa(0, "Spectator"), aa(1, "")))
		}
		return nil, nil
	case "GUI.openTab":
		if len(args) >= 1 && gui.safeEval != nil {
			...
			gui.safeEval(fmt.Sprintf(`_sOpenTab(%q,%q)`, aa(0, ""), grp))
		}
		return nil, nil
	case "GUI.css":
		if len(args) >= 3 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sCSS(%q,%q,%q)`, aa(0, ""), aa(1, ""), aa(2, "")))
		}
		return nil, nil
	case "GUI.addClass":
		if len(args) >= 2 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sClass(%q,%q,true)`, aa(0, ""), aa(1, "")))
		}
		return nil, nil
	case "GUI.removeClass":
		if len(args) >= 2 && gui.safeEval != nil {
			gui.safeEval(fmt.Sprintf(`_sClass(%q,%q,false)`, aa(0, ""), aa(1, "")))
		}
		return nil, nil
	case "GUI.eval":
		if len(args) >= 1 && gui.safeEval != nil {
